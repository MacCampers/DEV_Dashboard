<div id="match-timeline">
   ...
   <div class="timeline">
      <div class="step has-action" style="width: {{ ($project->step1_duration / $project->duration) * 100 }}%;">
         ...
         <div class="fill"></div>
         <div class="title">
            @if( $match->canUploadLoi() && !$match->loi )
               <a href="{{ route('match_upload_form', ['id' => $match->id, 'document' => 'loi']) }}"><span class="icon icon-add"></span>@lang('buttons.upload_loi')</a>
            @elseif( $match->loi )
               <a href="{{ route('match_view_document', ['id' => $match->id, 'document' => 'loi']) }}"><span class="icon icon-document"></span>@lang('buttons.view_loi')</a>
            @else
               <span class="icon icon-add"></span>@lang('buttons.upload_loi')
            @endif
         </div>
      </div>
      ...
      <div class="step has-action" style="width: {{ ($project->step3_duration / $project->duration) * 100 }}%;">
         <div class="fill"></div>
         <div class="title">
            @if( $match->canUploadBindingOffer() && !$match->binding_offer )
               <a href="{{ route('match_upload_form', ['id' => $match->id, 'document' => 'binding_offer']) }}"><span class="icon icon-add"></span>@lang('buttons.upload_binding_offer')</a>
            @elseif( $match->binding_offer )
            ...
            @else
               <span class="icon icon-add"></span>@lang('buttons.upload_binding_offer')
            @endif
         </div>
      </div>
